...
if($p=="boking" AND $action=="tambah-boking"){
	...
	$validasi = mysqli_query($conn, "SELECT * FROM tboking_temp WHERE idSession='$_SESSION[username]'");
	...
	window.location=('frame.php?p=keranjang-boking')
		...
		";
				...
				$jadwal=mysqli_query($conn, "SELECT * FROM tjadwal,tlapangan,tjam WHERE
	...
	";
		...
		mysqli_query($conn, "DELETE FROM tboking_temp WHERE idSession='$_SESSION[username]'");
		echo"
	<script language='javascript'>
	window.alert('Data Berhasil Disimpan');
...
$cekkeranjang = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tboking_temp WHERE idSession='$_SESSION[username]'"));
if ($cekkeranjang == 0){
...
$lunas="B";
$tot=mysqli_query($conn, "SELECT SUM(subTotalTemp) AS total FROM tboking_temp WHERE idSession='$_SESSION[username]'");
...
for ($i = 0; $i < $jml; $i++){
  mysqli_query($conn, "INSERT INTO trincian_boking(kdBoking,kdJadwal,noLapangan,tglBoking,jamBoking,hargaBoking,subTotal) 
