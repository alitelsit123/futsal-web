...
if(isset($_SESSION['username']) AND isset($_SESSION['password'])){
	...
	$item=mysqli_query($conn, "SELECT * FROM tjadwal,tlapangan,tjam WHERE tjadwal.kdLapangan=tlapangan.kdLapangan 
						   AND tjadwal.kdJam=tjam.kdJam AND tjadwal.kdJadwal='$_GET[id]'");
	$_data=mysqli_fetch_array($item);
	
	$SQL=mysqli_query($conn, "SELECT * FROM tboking_temp WHERE jamBokingTemp='$_data[jam]'");
	$ketemu=mysqli_num_rows($conn, $SQL);
	if($ketemu > 0){
		echo"
   ...
   <script language='javascript'>
	...
	window.location=('../../frame.php?load=boking&action=input')
		...
		mysqli_query($conn, "DELETE FROM tboking_temp WHERE kdbokingTemp=$_GET[id]")or die (mysqli_error());
	...
	";
		...
		mysqli_query($conn, "UPDATE tboking SET statusBayar='$_POST[rbStatus]' WHERE kdBoking='$_POST[id]'");
	echo"
	<script language='javascript'>
	...
	$cekkeranjang = mysqli_num_rows(mysqli_query("SELECT * FROM tboking_temp WHERE idSession='$_SESSION[username]'"));
if ($cekkeranjang == 0){
...
function isi_keranjang(){
	...
	$sql = mysqli_query($conn, "SELECT * FROM tboking_temp WHERE idSession='$sid'");
...
$query=mysqli_query($conn, "SELECT MAX(noInvoice) As nomorOrder FROM tboking");
...
$lunas="B";
$tot=mysqli_query($conn, "SELECT SUM(subTotalTemp) AS totalBayar FROM tboking_temp WHERE idSession='$_SESSION[username]'");
$r=mysqli_fetch_array($tot);


mysqli_query($conn, "INSERT INTO tboking (noInvoice,tglInvoice,usernameBoking,an,alamat,email,kontak,totalBayar,statusBayar)
...
for ($i = 0; $i < $jml; $i++){
  mysqli_query($conn, "INSERT INTO trincian_boking(kdBoking,noLapangan,kdJadwal,hargaBoking,jamBoking,tglBoking,subTotal) 
...
mysqli_query($conn, "UPDATE tjadwal SET statusBoking='B' WHERE kdJadwal='{$isikeranjang[$i]['kdJadwal']}'");

}

  mysqli_query($conn, "DELETE FROM tboking_temp
